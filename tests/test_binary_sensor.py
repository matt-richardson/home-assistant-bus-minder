...
async def test_connected_sensor_off_on_first_sse_failure(hass: HomeAssistant, mock_config_entry):
    """Connected sensor goes 'off' on the first SSE failure, not after the threshold."""
    mock_config_entry.add_to_hass(hass)
    await hass.config_entries.async_setup(mock_config_entry.entry_id)
    await hass.async_block_till_done()
